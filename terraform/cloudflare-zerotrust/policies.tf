resource "cloudflare_access_policy" "allow_status" {
  application_id = cloudflare_access_application.status.id
  zone_id        = var.cloudflare_zone_id
  name           = "Network Status Policy"
  precedence     = "1"
  decision       = "allow"
  dynamic "include" {
    for_each = var.allow_status_list
    content {
...
resource "cloudflare_access_policy" "allow_clancy_admin" {
  application_id = cloudflare_access_application.clancy_admin.id
  zone_id        = var.cloudflare_zone_id
  name           = "Clancy Admin Policy"
  precedence     = "1"
  decision       = "allow"
  dynamic "include" {
    for_each = var.allow_clancy_admin_list
    content {
      email = include.value.email
